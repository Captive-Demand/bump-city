...
const StepDots = ({ current, total }: { current: number; total: number }) => (
  <div className="flex items-center gap-2 justify-center mb-8">
    {Array.from({ length: total }).map((_, i) => (
      <div key={i} className={cn("h-2 rounded-full transition-all", i === current ? "w-8 bg-primary" : i < current ? "w-2 bg-primary/60" : "w-2 bg-muted")} />
    ))}
  </div>
);
...
const RegistrySetupPage = () => {
  ...
  const handleFinish = async () => {
    ...
    const { error } = await supabase.from("events").insert({
      ...
      city: city.trim() || null,
      registry_name: registryName.trim() || `${honoreeName.trim()}'s Baby Registry`,
    ...
    const data: Partial<SetupData> = {
      honoreeName: honoreeName.trim(), dueDate, city: city.trim(),
        ...
        <div className="flex-1">
          {step === 0 && (
            <div className="space-y-5">
              <div><h2 className="text-2xl font-bold">About You</h2><p className="text-sm text-muted-foreground mt-1">Let's personalize your registry.</p></div>
              <div className="space-y-1.5">
                <Label htmlFor="name">Your name(s) *</Label>
                <Input id="name" placeholder="e.g. Sarah & Mike" value={honoreeName} onChange={(e) => setHonoreeName(e.target.value)} maxLength={100} />
              </div>
              <div className="space-y-1.5">
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div><h2 className="text-2xl font-bold">Registry Preferences</h2><p className="text-sm text-muted-foreground mt-1">Customize how your registry works.</p></div>
              <div className="space-y-1.5">
                <Label htmlFor="regname">Registry name</Label>
                <Input id="regname" placeholder="e.g. Baby Smith Registry" value={registryName} onChange={(e) => setRegistryName(e.target.value)} maxLength={100} />
              </div>
              <div>
                ...
                <RadioGroup value={giftPolicy} onValueChange={(v) => setGiftPolicy(v as typeof giftPolicy)} className="space-y-3">
                  {[
                    ...
                    <Label key={opt.value} htmlFor={`reg-${opt.value}`} className={cn("flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all", giftPolicy === opt.value ? "border-primary bg-primary/5" : "border-border")}>
              ...
              <div>
                ...
                <RadioGroup value={registryPrivate ? "private" : "public"} onValueChange={(v) => setRegistryPrivate(v === "private")} className="space-y-3">
                  {[
                    ...
                    <Label key={opt.value} htmlFor={`share-${opt.value}`} className={cn("flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all", (opt.value === "private") === registryPrivate ? "border-primary bg-primary/5" : "border-border")}>
