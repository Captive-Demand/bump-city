...
import { ChevronLeft, Send, Users, Gift, Calendar, MapPin, Sparkles, CalendarPlus, Navigation } from "lucide-react";
...
import { ShowerSettingsSheet } from "@/components/shower/ShowerSettingsSheet";
...
import { useShowerStats } from "@/components/shower/useShowerStats";
...
const ShowerDetailPage = () => {
  ...
  const { allEvents, activeEvent, switchEvent, loading } = useActiveEvent();
  const { user } = useAuth();
  const { isImpersonating, impersonatedRole } = useRoleContext();
  ...
  let isHost = isOwner || memberRole === "host" || memberRole === "co-host";

  // Impersonation overrides: when an admin previews "as Guest" / "as Honoree",
  // hide the host-only UI on this page too. Without this, the page bypasses
  // RoleContext and admins can't see what guests actually see.
  if (isImpersonating && impersonatedRole) {
    isHost = impersonatedRole === "host" || impersonatedRole === "co-host" || impersonatedRole === "admin";
  }

  return <ShowerDetailContent event={event} isHost={isHost} navigateBack={() => navigate("/showers")} />;
};

const ShowerDetailContent = ({
  event,
  isHost,
  navigateBack,
}: {
  event: ReturnType<typeof useActiveEvent>["activeEvent"] & {};
  isHost: boolean;
  navigateBack: () => void;
}) => {
  const navigate = useNavigate();
  const stats = useShowerStats(event.id, event.city);
  ...
  return (
    <MobileLayout>
      ...
      <div className="px-6 pb-8 space-y-6">
        {isHost ? (
          <>
            <ShowerStatusStrip event={event} stats={stats} />
              ...
              steps={[
                {
                  ...
                  description: "Pick a template and add your details.",
                  done: !!event.invite_image_url,
                },
                {
                  number: 2,
                  icon: Users,
                  title: "Add your guests",
                  description: "Import contacts and send invitations.",
                  done: stats.guests > 0,
                },
                {
                  number: 3,
                  ...
                  description: "Add gifts you'd love to receive.",
                  done: stats.registry > 0,
                },
              ]}
            />

            <InvitationOptionsCard event={event} unsentInviteCount={stats.invitesUnsent} />
            <ManageTilesGrid eventId={event.id} city={event.city} stats={stats} />
            <ShowerSettingsSheet event={event} />
          </>
        ) : (
          <>
